...
export function ReportModal({ onClose }: ReportModalProps) {
  ...
  const fetchTodayReport = useCallback(async () => {
    ...
    try {
      const response = await axios.get<ReportData>('http://localhost:3000/api/patient/reports/today', {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (existingReportId) {
        ...
        await axios.put('http://localhost:3000/api/patient/reports/today', 
          reportPayload,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        ...
        await axios.post('http://localhost:3000/api/patient/reports', 
          reportPayload,
          { headers: { Authorization: `Bearer ${token}` } }
        );
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete('http://localhost:3000/api/patient/reports/today', {
        headers: { Authorization: `Bearer ${token}` }
      });
